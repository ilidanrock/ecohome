...
- **Tenant assigned properties**: Tenants see their assigned properties via GET `/api/tenant/rentals` (ListRentalsForTenant use case). Dashboard section "Mis propiedades asignadas" and page `/dashboard/properties`; sidebar link "Mis propiedades". Only rentals whose property exists (not soft-deleted) are returned. Hook `useTenantRentalsQuery()` in `lib/queries/rentals.ts`.
- **Delete property cascades to rentals**: When an admin soft-deletes a property, all rentals of that property are soft-deleted first (`IRentalRepository.softDeleteByPropertyId`), then the property. This avoids orphan rentals so tenants do not see "—" for deleted properties. DeleteProperty use case takes `rentalRepository` and calls `softDeleteByPropertyId(propertyId, userId)` before `propertyRepository.softDelete`.
- **Responsive tenant dashboard**: Layout uses flex so sidebar and content share space correctly; `useIsMobile()` defaults to `true` so on first paint mobile gets the Sheet sidebar (menu button works immediately). Tenant sidebar closes the Sheet on pathname change so it does not stay open over content.
...
- **E2E properties spec**: Assign-tenant test uses calendar popover (`[data-state="open"]` + `select`) for month/year to avoid targeting the tenant combobox; delete-property test creates a dedicated property via API at test start and asserts row disappearance so it passes on retries (CI only re-runs the failed test). Deleting a property in the app also soft-deletes its rentals (backend).
